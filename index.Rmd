...
\svp{Cross-correlation between two signatures $S_1(x)$ and $S_2(x)$ is defined as $$CC(S_1, S_2, z) = \text{cor}\left(S_1(x), S_2(x + z)\right)$$ where $z$ is the shift difference between the two sequences. We maximize $CC$ over values of $z$ to determine the best alignment  \citep{aoas,vorburgerApplicationsCrosscorrelationFunctions2011}; this maximal value is then used to assess the match strength between the two sequences. When signatures contain missing values, pairwise complete observations are used to calculate the cross-correlation.}
