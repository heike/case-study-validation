...
### Comparison of land-to-land scores

\hh{XXX error rates on land to land matches? XXX}
...
```{r rocs, fig.caption="ROC curves for all studies based on land-to-land comparisons."}
library(plotROC)
library(pROC)
all_lands %>% 
  ggplot(aes(colour = interaction(study, group1))) +
  geom_roc(aes(m = rfscore, d = as.numeric(sameland)), 
           labels =FALSE, pointsize = 0) 
```
