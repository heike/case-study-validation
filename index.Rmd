...
Interestingly, for consecutively matching striae, an equal error rate is reached for just two matching striae, which from a practical point of view is not sustainable. A  CMS of  6 or above is an established cutoff used in identifications. This leads to 16 wrong identifications (out of 49286; for an error rate of about 3 in 10,000) but only identifies 115 out of  715 known same-source lands, for an error rate of 16 in 100. XXX check after file names are corrected. }
```{r eval=FALSE}
xtabs(~sameland+I(cms>=6), data = all_lands)
```
