...
h44nest <- h44nest %>% mutate(
  data = data %>% purrr::map(
    .f = function(d) {
      ...
      d$sameland = bullet_to_land_predict(
        land1 = d$land1, land2 = d$land2, 
        d$rfscore, difference=0.01, alpha =1)
...
pdnest <- pdnest %>% mutate(
  data = data %>% purrr::map(
    .f = function(d) {
      d$sameland = bullet_to_land_predict(
        land1 = d$land1, land2 = d$land2, 
        d$rfscore, difference=0, alpha =1)
      d
    })
)
...
library(pROC)
all_lands %>% 
  ggplot(aes(colour = interaction(study, group1, sep = " "))) +
  geom_roc(aes(m = rfscore, d = as.numeric(sameland)), 
...
```{r why, fig.cap="Density plots for all measures "}
all_lands %>% gather(measure, score, ccf, cms, rfscore) %>%
ggplot(aes(x = score, fill = interaction(measure, sameland))) + 
  geom_density(alpha = 0.5) +
  scale_fill_brewer(palette = "Paired") +
  facet_grid(measure~study+group1, scales="free_x") 
```
