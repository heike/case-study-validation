...
all_lands <- rbind(
  h44lands %>%
    select(rfscore, ccf, sameland, samesource) %>%
    mutate(study = "Hamby 44", group1 = ""),
  pdlands %>%
    select(rfscore, ccf, sameland, samesource) %>%
    mutate(study = "Phoenix PD", group1 = ""),
  houlands %>%
    select(rfscore, ccf, sameland, samesource, group1) %>%
    mutate(study = "Houston FSI")
)
...
print(xtable(aucs %>% select(-data), digits = 3, caption = "Area under the curve (AUC) for each (set) of the studies. At the land level there is no difference in prediction power between RF score and cross-correlation."), include.rownames=FALSE, label="tab:aucs")
```
...
\begin{tabular}{llrr}
  \hline
\bf Study & \bf Set & \bf AUC(RF score) & \bf AUC(Cross-correlation) \\ 
  \hline
\bf Hamby 44 &  & 0.956 & 0.926 \\ 
\bf   Phoenix PD &  & 0.976 & 0.986 \\ 
\bf   Houston FSI & G1 & 0.991 & 0.988 \\ 
\bf   Houston FSI & G2 & 0.898 & 0.902 \\ 
\bf   Houston FSI & G3 & 0.986 & 0.987 \\ 
   \hline
\end{tabular}
\caption{Area under the curve (AUC) for each (set) of the studies. As seen in the ROC curves, the algorithm performs the least well on set 2 of the Houston FSI study. Overall the performance on all sets is very good to excellent.
At the land level there is no difference in the predictive power between RF score and cross-correlation. \label{tab:aucs}} 
\end{table}

# Discussion and Conclusions


What is quite surprising is that what we anticipated to be the easiest case of Hamby 44 turned out to be the hardest to solve. This is likely due to several lands in Hamby set 44 that have major deficiencies,  such as 'tank rash' (a collision of the bullet after exiting the barrel with a surface causing markings on top of the striations from the barrel) or extreme pitting (holes caused by direct contact with burning gun powder).
