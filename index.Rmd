...
```{r rocs, fig.cap="ROC curves for all studies based on land-to-land comparisons of the random forest score.", fig.width = 6, fig.height = 10, out.width = ".5\\textwidth"}
library(plotROC)
library(pROC)
rocpic <- all_lands %>% gather(measure, score, rfscore, ccf, cms) %>%
  ...
  theme_bw() +
  facet_wrap(~study+group1, nrow=3) +
...
```{r aucs-pic, fig.width=6, fig.height = 6, out.width='.5\\textwidth', fig.cap=""}
...
aucpic <- aucs %>% 
  mutate(
    measure = factor(measure, levels = c("cms", "ccf", "rfscore")),
    measure = factor(measure, labels = c("CMS", "CC", "RF"))
  ) %>%
  ggplot(aes(x = auc, y = measure, colour = interaction(study, group1), linetype=measure)) +
  facet_grid(study+group1~.) +
  geom_errorbarh(aes(xmin=auc_ci_low, xmax=auc_ci_high), size=1, height=0.3) +
  geom_point() +
  theme_bw() +
  ...
  theme(legend.position = "bottom",
        legend.key.width = unit(1.5, "cm")) +
  scale_linetype_manual("Measure", values = c(3,1,2))
