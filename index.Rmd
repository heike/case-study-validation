...
The cross-correlation (CC) function between two signatures $S_1(x)$ and $S_2(x)$ is defined as \[CC(S_1, S_2, z) = \text{cor}\left(S_1(x), S_2(x + z)\right)\] where \hh{$x$ and $x+z$ are integer values appropriately defined within the domains of $S_1$ and $S_2$ and  $z$ is the lag between the two sequences. The lag $z$ used to achieve maximum $CC$ is used to determine the best alignment; the maximal value of $CC$ is  used to assess the match strength between the two sequences.} When signatures contain missing values, pairwise complete observations are used to calculate the cross-correlation.
...
Note that because signatures are moved into phase based on the maximum cross-correlation rather than matching striation marks, it is possible for two lands to have a consecutive matching striae of 0. CMS are generally considered to be a very conservative criteria, in that bullets fired through the same barrel may not have a high number of consecutively matching striae; in this sense, CMS may be more likely to produce an inconclusive result.
...
The random forest presented in \citet{aoas} is based on a combination of multiple characteristics, such as maximized cross-correlation, number of matching striae, and number of consecutively matching striae among others. The output of the random forest is a score between 0 and 1 representing the algorithm's assessment of the strength of the match, where scores close to 1 indicate that the algorithm believes it to be highly likely that the aligned signatures come from the same source, while low scores indicate that the aligned signatures are highly likely to originate from different lands.
...
\end{table}
\hh{Note that this definition of error is more stringent than the AFTE Theory of Identification, in that it does not allow for inconclusive results, but forces a decision based on a cutoff value. } \svp{This increased rigor may increase the error rate of the model when compared to examiner error rates, but we expect that the increased information available from the 3D measurement data will compensate for this loss. A binary decision model is also easier to interpret and provides more clear-cut, definitive results.} 
...
\svp{When evaluating the \citet{aoas} algorithm, we will describe errors as a percentage of correct evaluations. Some machine learning tools, such as ROC curves and AUC, will be presented in the decimal (rate) form because they do not easily translate into a percent-form interpretation. }

\svp{XXX Should we have some sort of header here to differentiate it from the error section?}
