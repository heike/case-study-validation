...
One of the measures used by firearms examiners to describe the strength of a match quantitatively is the number of consecutive matching striae (CMS) as defined by \citet{biasotti}.
If there are at least 6 consecutive matching striae, the bullets are considered to be a good match \citep{biasotti_firearms_1997,nichols_scientific_2006}, according to criteria used by FTEs. 
CMS in this setting are counted as the number of consecutive peaks in signatures of two lands that match. 


\hh{HH: let's not go into what we do not do, and instead focus on what we do}
~~  There are some differences between CMS as defined by FTEs and the method used in \citet{aoas} includes both peaks and valleys, as it is easier to assess the features separately using cross-sections. XXX Should we go into this, or just leave it as is?~~
...
\hh{XXX that's what I thought, too.}
