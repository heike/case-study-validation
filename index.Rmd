...
\] 
where $x$ and $x+z$ are integer values appropriately defined within the domains of $S_1$ and $S_2$, $z$ is the lag between the two sequences, \svp{and }\hh{$\text{cor}(.,.)$ is the Pearson correlation coefficient.}
...
Note that the sequence average maximum of the correlation between lands is used in SensoComp \hh{citation for that?}\svp{It's your claim... I looked for a citation and didn't find one. Closed-source proprietary = no references?} to capture the similarity between bullets. The correlation based SAM score has also been called the 'average correlation calculated at the max phase' in \citet{chuPilotStudyAutomated2010}. 
