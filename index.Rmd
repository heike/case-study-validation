...
Quantitative features describing the similarity of two signature are extracted. Features include characteristics of the signature that firearms and toolmark examiners base their visual assessment on, such as  depth of striae, number of matching striae, and the number of consecutively matching striae (CMS). We also include statistical features, such as cross-correlation and Euclidean distance between the signatures.

# Validation Sets

The algorithm in \citet{aoas} is trained on scans from Hamby sets 252 and 173 made available through \citet{nist}. Set 173 was originally published as Hamby set 44. This has been rectified now.
...
f3 <- rbind(
  f2 %>% select(b1, b2, barrel1, bullet1, 
                land1, bullet2, land2, ccf, rfscore, KM) %>%
    mutate(barrel2 = "Unknown"),
  unknowns %>% select(b1, b2, bullet1, 
...
This is likely due to several lands in the Hamby-44 set having major deficiencies,  such as 'tank rash' (a collision of the bullet after exiting the barrel with a surface causing markings on top of the striations from the barrel) or extreme pitting (holes caused by direct contact with burning gun powder).
\autoref{fig:tankrash} gives an overview of all lands of bullets in the Hamby-44 set affected. 
...
knitr::include_graphics(
  c('./images/B1-B1-L6.png', './images/B2-B2-L5.png',
...
Three lands from bullets known to be fired from barrel 8 are affected. This is the likely cause for the low scores for barrel 8 in \autoref{fig:h44}. We also see that questioned bullet 'E' is affected, explaining some of the low similarity scores for this bullet.
