...
all_lands %>% 
  ...
  scale_colour_manual("Study", values = c("#fe9929", "#8856a7",
...
\bf   Houston FSI & G3 & 0.986 & 0.987 \\ 
   \hline
\end{tabular}
\caption{Area under the curve (AUC) for each (set) of the studies. As seen in the ROC curves, the algorithm performs the least well on set 2 of the Houston FSI study. Overall the perform on all sets is very good to excellent.
At the land level there is no difference in prediction power between RF score and cross-correlation.\label{tab:aucs}} 
\end{table}
...
\autoref{fig:tankrash} gives an overview of all lands of bullets in the Hamby-44 set affected. These issues affect the algorithm's ability to identify a stable signature, which impacts the subsequent match scores.
...
### Limitations
All of the studies involve the same type of firearm. We see some variations in scores across different models of Rugers. Likely, we are going to see even bigger variations with different brands of firearms, where there might be different number of lands of different lengths. We also know  that different firearms mark differently well \cite{???}. Rugers are among the firearms that mark very well, so we would correspondingly expect to see lowered similarity scores for other firearms. The matching algorithm works on matching lands. Barrels with polygonal rifling, such as Glocks, do not introduce well defined lands on bullets, which makes an application of the matching algorithm impossible.  
