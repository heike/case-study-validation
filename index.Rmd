...
\svp{Once signatures have been extracted from stable regions with expressed striae, they must be aligned in order to assess their match strength. \citet{aoas} align signatures by maximizing the cross-correlation between the signatures. }

\svp{The cross-correlation between two signatures $S_1(x)$ and $S_2(x)$ is defined as $$CC(S_1, S_2, z) = \text{cor}\left(S_1(x), S_2(x + z)\right)$$ where $z$ is the shift difference between the two sequences. We maximize $CC$ over values of $z$ to determine the best alignment  \citep{aoas,vorburgerApplicationsCrosscorrelationFunctions2011}; this maximal value is then used to assess the match strength between the two sequences. When signatures contain missing values, pairwise complete observations are used to calculate the cross-correlation.}
...
\svp{After two signatures are aligned, q}uantitative features describing the similarity of two signature are extracted. Features include characteristics of the signature that firearms and toolmark examiners base their visual assessment on, such as striae depth, number of matching striae, and the number of consecutively matching striae (CMS). Statistical features such as cross-correlation and Euclidean distance between the signatures \svp{are also extracted and used to assess the strength of the correspondance between the aligned signatures. }
...
\svp{The random forest presented in \citet{aoas} takes as input the following characteristics derived from a pair of aligned sequences:
\begin{itemize}
\item Cross-correlation 
\item Number of matching striae 
\item Total depth of joint striations 
\item Number of non-matching striae 
\item Euclidian distance between the sequences 
\item Consecutive Matching Striae 
\item Consecutive Non-matching Striae
\end{itemize}

The output of the random forest is a score between 0 and 1 representing the algorithm's assessment of the strength of the match, where 1 indicates that the algorithm believes it to be highly likely that the aligned signatures match, and 0 indicates that the aligned signatures are highly likely to originate from different lands.}

# Assessing Match Strength
\svp{Each of the features described in the previous section, as well as the random forest score, describe comparisons on a land-to-land basis. While these comparisons are useful, it is typically more useful to assess the entire physical object (e.g. all lands on a single bullet) to describe the correspondance between two pieces of physical evidence. In order to aggregate land-to-land match scores, we must consider the physical structure of the evidence; that is, that the sequence of lands stems from spatial information, but the numbering of each land represents the scan order and start point rather than encoding useful information. }
