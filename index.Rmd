...
\hh{XXX error rates on land to land matches? ROC curves in \autoref{fig:rocs} and corresponding AUC results for all studies in \autoref{tab:rocs} XXX}
...
```{r rocs, fig.cap="ROC curves for all studies based on land-to-land comparisons.", fig.width = 4.5, fig.height = 3, out.width = ".5\\textwidth"}
library(plotROC)
library(pROC)
all_lands %>% 
...
At the land level there is no difference in prediction power between RF score and cross-correlation. \svp{It has been observed \citep{marzban_roc_2004} that AUC differentiates good models from bad, but does not effectively distinguish between good models to determine which model is better.} \label{tab:aucs}} 
\end{table}
