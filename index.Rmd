...
\item[(c)] Remove bullet curvature \hh{using a Gaussian filter\cite{nist} or non-parametric smooth \citep{aoas}}. 
\end{itemize}
...
The cross-correlation function (ccf) between two signatures $S_1(x)$ and $S_2(x)$ is defined as $$CC(S_1, S_2, z) = \text{cor}\left(S_1(x), S_2(x + z)\right)$$ where \hh{$x$ and $x+z$ are integer values appropriately defined within the domains of $S_1$ and $S_2$ and }  $z$ is the lag between the two sequences. We maximize $CC$ over values of $z$ to determine the best alignment  \citep{aoas,vorburgerApplicationsCrosscorrelationFunctions2011}; the maximal value of $CC$ is  used to assess the match strength between the two sequences. When signatures contain missing values, pairwise complete observations are used to calculate the cross-correlation.
