...
\item[(c)] Remove bullet curvature \hh{using a Gaussian filter \cite{chu?} or non-parametric smooth \citep{aoas}}. 
\end{itemize}
...
\hh{XXX for CMS we need to define peaks, and matching peaks, and for that we need to define the additional filter/smooth.}
...
\hh{We need to think what to do about the list - we could either just say that the random forest is based on multiple features, such as, ... and list three (and refer to the paper). Or we expand quite a bit and define the features in mathy detail using our notation above.}
\svp{\begin{itemize}
...
Each of the features described in the previous section, as well as the random forest score, describe comparisons on a land-to-land basis. While these comparisons are useful, \hh{the question of interest typically involves }  the entire physical object (e.g. all lands on a single bullet).
