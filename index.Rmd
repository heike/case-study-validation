...
3D characterization of bullet surfaces has been considered since 1958 \citeauthor{davis1968introduction}, but the technology was not sufficiently advanced for analysis based on 3D measurements to be a viable option. Approaches to utilizing 3D measurement data were proposed starting in 1999 \citeauthor{dekinderAutomatedComparisonsBullet1999}, and have been further developed by \citet{bachrachDevelopment3DbasedAutomated2002,xieAutomatedBulletidentificationSystem2009, chuPilotStudyAutomated2010}. These approaches utilize 3D surface measurements directly to characterize the topology of bullet surfaces, rather than using visual or image comparison techniques. In many cases, they also provide some level of automation of the comparison process, with the goal of reducing human biases by augmenting the visual information with 3D measurements. This allows for examination not only of the peaks in the bullet signature, but also for comparison of the valleys, which are more difficult to assess visually except in relation to the peaks. 

Commonly, approaches derived from 3D surface measurements use some features which are developed to mimic the perception of FTEs\citep{luAutomatedBulletIdentification2014}. Class characteristics, which are shared by a group of firearms with the same rifling design, manufacturer, and tooling process, are typically evaluated first, as a mismatch on class charcteristics is sufficient for an exclusion. Automated approaches to estimation of width of the land engraved area and twist angle were described in \citet{chuPilotStudyAutomated2010}. Individual characteristics, which are not shared by all members of a class, can also be automatically assessed from 3D surface measurements; in some cases, these features mimic features used by FTEs during image analysis. One of the most common features used to assess the strength of a match based on individual characteristics is the cross-correlation function (ccf), which is described in several studies \citep{maNISTBulletSignature2004, vorburgerApplicationsCrosscorrelationFunctions2011, chuPilotStudyAutomated2010}. Additional features proposed for automatically assessing match strength also include signature distance \citep{maNISTBulletSignature2004}, striae depth and width \citep{ComputerIdentificationBullets1978}, and consecutive matching striae \citep{chuAutomaticIdentificationBullet2013}.

In the matching algorithm proposed in \citet{aoas}, several of the features described above are combined to produce a similarity score based on 3d topographic scans of land engraved areas (LEAs). From each land engraved area a signature is derived by
...
The algorithm in \citet{aoas} is trained on scans from Hamby sets 252 and 173 made available through \citet{nist}. Set 173 was originally published as Hamby set 44. This mislabelling has been correct now. The scans used to train the model were taken at 20 fold magnification for a resolution of 1.5625 microns per pixel. \hh{While magnification is generally of interest in microscopy, {\it resolution} -- usually measured in microns per pixel -- is of more interest for 3D topographic measurements, as it determines the operative level of available data.}

\hh{XXX We need a gentler intro to the validation sets - maybe some discussion of training and test sets?}
...
Scans of all land engraved areas were taken on a Sensofar Confocal Light microscope at 20x magnification resulting in a resolution of 0.645 microns per pixel. If not indicated otherwise, scans were taken at the Roy J Carver high resolution microscopy lab at Iowa State University.
...
The Phoenix PD set uses Ruger XXX barrels, which are different from the barrels in the Hamby sets used for training the algorithm, but the barrels are rifled similarly to the Ruger P-85 barrels.  
...
\autoref{fig:pd} shows an overview of similarity scores for all pairs of questioned bullets and  test fires. The color encoding is the same as in the previous figures. \svp{Here, we see that some of the questioned bullets match all 3 bullets fired out of the same barrel.} None of the test fires from barrel U10 \svp{match} any of the questioned bullets. Similarly, questioned bullets 'Q', 'Y', and 'Z' do not match any of the known barrels. This is a sign of the previously mentioned open set characteristic of the study. \svp{Once again, the results from automatic matching correctly pair questioned bullets with their corresponding barrels.}

## Houston FSI

\autoref{fig:hou} shows an overview the three sets of the scores for all pairs of questioned bullets with all other bullets \svp{in the set. In set 1, five of the questioned bullets can be matched to known bullets in the set. None of the questioned bullets in set 1 match bullets fired from barrels KC or KD. Additionally, two of the questioned bullets which do not match any of the barrels in the set match each other. In set 2, four of the questioned bullets can be matched to known bullets in the set; of the additional four questioned bullets which do not match any known bullets, two were fired from the same barrel. In set 3, six of the questioned bullets match known bullets, and the remaining two questioned bullets match each other but do not match any known bullets in the set. Thus, we can conclude that the algorithm performs well on open set studies, as it can identify matches among questioned bullets which do not match any known bullets.}
...
However, SAM scores based on the multivariate random forest score show better separation between same-source and different-source scores than SAM scores based on cross-correlation. This can be seen from the larger horizontal distance between the modes\svp{(peaks)} of the density curves of RF based SAM scores \svp{compared to the} cross-correlation SAM scores.  

Regarding requirement (R2), \autoref{fig:compare} shows that \svp{a single cutoff value does not exist that separates same-source scores from different-source scores across all three studies, without introducing additional errors}.

There are several approaches for potential improvements: we can try to fine tune the matching algorithm to take make and model of the firearm and ammunition used into account or expand the trainings base of the algorithm to include a wider variety of makes and models. The downside of this is that we need to considerably expand the database used for training. 
...
What is quite surprising \svp{is} that what we anticipated to be the easiest case of Hamby 44 turned out to be the hardest to solve. This is likely due to several lands in the Hamby-44 set \svp{that have} major deficiencies,  such as 'tank rash' (a collision of the bullet after exiting the barrel with a surface causing markings on top of the striations from the barrel) or extreme pitting (holes caused by direct contact with burning gun powder).
\autoref{fig:tankrash} gives an overview of all lands of bullets in the Hamby-44 set affected. \svp{These issues affect the algorithm's ability to identify a stable signature, which impacts the subsequent match scores.}
```{r tankrash, fig.cap="Overview of bullet lands with prominent deficiency such as tank rash or extreme pitting in the Hamby-44 study.", fig.width=4, fig.height = 2, out.width='47.5%', fig.subcap=c("Br1-B1-L6 (breakaway)", "Br2-B2-L5 (tank rash)", "Br3-B1-L5 (tank rash)", "Br8-B1-L6 (tank rash)", "Br8-B2-L2 (pitting)", "Br8-B2-L6 (breakaway)", "Unk-E-L6 (breakaway)", "Unk-I-L6 (breakaway)"), fig.align="middle", fig.ncol=2}
...
Three lands from bullets known to be fired from barrel 8 are affected\svp{, producing} low scores for barrel 8 in \autoref{fig:h44}. We also see that questioned bullet 'E' is affected, explaining some of the low similarity scores for this bullet.\svp{You've marked I vs. 5 and F vs. 8 in \autoref{fig:compare}; should E be marked?}

### Limitations
All of the studies involve the same firearm. We see some variations in scores across different models of Rugers. Likely, we are going to see even bigger variations with different brands of firearms, where there might be different number of lands of different lengths. We also know  that different firearms mark differently well \cite{???}. Rugers are among the firearms that mark very well, so we would correspondingly expect to see lowered similarity scores for other firearms. The matching algorithm works on matching lands. Barrels with polygonal rifling, such as Glocks, do not introduce well defined lands on bullets, which makes an application of the matching algorithm impossible.  


As seen in the earlier discussion, similarity scores are affected by deficiencies on lands. It has been shown \citet{lpr} that parts of lands can be used for successful identifications. The  algorithm proposed by \citet{aoas} is not capable of  automatically detecting parts of lands with well expressed striae. XXX citation for neural network algorithm
