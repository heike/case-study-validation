...
\svp{Firearms examiners often use consecutive matching striae (CMS) to describe the strength of a match. CMS as used by examiners counts only peaks as striae, as it is difficult to assess vallleys and peaks separately when using a comparison scope.  If there are at least 6 consecutive matching striae, the bullets are considered to be a good match \citep{biasotti_firearms_1997,nichols_scientific_2006}, according to criteria used by FTEs. There are some differences between CMS as defined by FTEs and the method used in \citet{asos} includes both peaks and valleys, as it is easier to assess the features separately using cross-sections. XXX Should we go into this, or just leave it as is?
}

\svp{Cross-correlation between two signatures $S_1(x)$ and $S_2(x)$, $$CC(S_1, S_2, z) = \lim_{L\rightarrow\infty}\left(\frac{1}{L} \int_{-L/2}^{L/2} S_1(x)S_2(x + z) dx \right)/[R_1R_2]$$ where $z$ is the shift difference between the two sequences and $R_1$ and $R_2$ are the root mean square roughness for the two signature \citep{vorburgerApplicationsCrosscorrelationFunctions2011}. We maximize $CC$ over values of $z$ to determine the best alignment; this maximal value is used to assess the match strength between the two sequences. XXX algorithm for calculating with missing values?}

\svp{XXX No idea what to say about the RF other than what's above?}

\hh{We already have a start on this above, but we should focus on these three measures in more detail.}

\hh{In the result section we should probably also include a discussion of EER and the threshold for EER for the three measures. Another threshold of interest is CMS=6, because that's what FTEs use often. }
...
\item {\bf Phoenix PD}
Tylor Klep from Phoenix PD provided sets of known test fires and questioned bullets: the set of known bullets consists of three test fires (B1, B2, B3) from each of eight different Ruger P-95 barrels (A9, C8, F6, L5, M2, P7, R3, U10). Ten questioned bullets were provided (B, E, H, J, K N, Q, T, Y, Z). This set is an open set; that is,\ it is not known in advance whether all (or any) of the questioned bullets are fired from the known barrels.
\svp{Need to add ammo info here...}
...
When evaluating classifier performance, it is common to use a receiver operating curve, which plots the false positives against the true positives for each possible value of the cutoff between the two classes (in this case, known match and known non-match). The ROC curves for the land-to-land scores are shown in \autoref{fig:roc-auc} (a). A perfect classifier would have 100\% true positives and 0\% false positives, e.g. the ROC curve would be a right angle at (0, 1). Classifiers with better performance will be closer to this corner of the plot. A random classifier would have an ROC curve that was a straight diagonal line through (0,0) and (1,1).
...
\hh{\autoref{fig:eer} shows an overview of Equal Error Rates (EER) based on the ROC curves of figure \ref{fig:roc-auc}. EERs are error rates when sensitivity and specificity of a test are equal, i.e. we see the same error rates of missing an identification for a land-to-land comparison  and wrong identifications, when it is determined that two land engraved areas come from the same source when they are, in fact, known to be from a different source. Equal error rates based on CMS of two lands are significantly higher than equal error rates based on cross-correlation or the RF score. At best, RF score and cross-correlation have an equal error rate of around 5\% for land-to-land comparisons. 
Interestingly, for consecutively matching striae, an equal error rate is reached for just two matching striae, which from a practical point of view is not sustainable. A  CMS of  6 or above is an established cutoff used in identifications. This leads to 0 wrong identifications but only identifies 21 out of  715 known same-source lands. XXX check after file names are corrected. }
```{r eval=FALSE}
xtabs(~sameland+I(cms>=12), data = all_lands)
```

```{r eer, fig.cap="Equal Error Rates for all measures (RF score, Cross-correlation and CMS) under all five (sub-)studies. CMS is signifcantly worse than both Cross-correlation and the RF score. At best, Cross-correlation and RF score have errors for land-to-land comparison of around 5%.", out.width = '.5\\textwidth'}
aucs %>% ggplot(aes(y = studyg, x = eer, colour=measure, shape = measure)) + 
  geom_point(size = 3) +theme_bw() +
  xlim(c(0, .35)) +
  xlab("Equal Error Rates (EER)") +
  ylab("") +
