...
\svp{Firearms examiners often use consecutive matching striae (CMS) to describe the strength of a match. CMS as used by examiners counts only peaks as striae, as it is difficult to assess vallleys and peaks separately when using a comparison scope.  If there are at least 6 consecutive matching striae, the bullets are considered to be a good match \citep{biasotti_firearms_1997,nichols_scientific_2006}, according to criteria used by FTEs. There are some differences between CMS as defined by FTEs and the method used in \citet{aoas}. The computational method includes counts both peaks and valleys as striae, as it is easier to assess the features separately using cross-sections than it is to do so usingn visual examination. XXX Should we go into this, or just leave it as is?
}
