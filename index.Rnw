...
\title{Case study validations of automatic bullet matching\tnoteref{t1}\tnoteref{t2}}
...
\tnotetext[t2]{The authors wish to thank Alan Zheng (NIST), Tylor Klep (Phoenix PD), and Melissa Nally and Kasi Kirksey (FSI Houston) for access to test set bullets. We would also like to thank the efforts of the Roy J Carver High Resolution lab in scanning the bullet sets and providing the scans to us. Finally, thanks to Prof James Hamby for his advice on XXX.}
...
\author[isu,csafe]{Heike Hofmann\corref{corauthor}}
\cortext[corauthor]{Corresponding author}\ead{hofmann@iastate.edu}
\author[isu,csafe]{Susan Vanderplas}\ead{srvander@iastate.edu}

\address[iastate]{Statistics Department, Iowa State University\\2438 Osborne Dr, Ames, IA 50011}
...
<<sam-sketch, out.width='\\textwidth', fig.width=12, fig.height = 2.5, fig.cap="Sketch of all six land-to-land sequences between two bullets with six lands.">>=
...
<<h44, echo=FALSE, fig.width = 10, fig.height=5, fig.cap="Hamby set 44: overview of all bullet-to-bullet matches between all pairs of questioned bullets (y-axis) and known test fires from ten barrels (x axis).">>=
...
<<pd, echo=FALSE, fig.width = 9, fig.height=4, fig.cap="Phoenix study: overview of all bullet-to-bullet matches between all pairs of questioned bullets (y-axis) and known test fires from ten barrels (x axis). The order of the bullets on the y axis is determined by matching barrel.">>=
...
<<hou, fig.width=9, fig.height = 3, fig.cap="Overview of matching scores for all pairs of questioned bullets to known bullets from five barrels (on the left) and questioned bullets to themselves (tiles on the right).", fig.subcap=c("Set 1", "Set 2", "Set 3"), fig.align="middle", fig.ncol=1>>=
features <- read.csv("data/fsi-features.csv.gz")
features <- features %>% mutate(
  lys = overlap*signature_length*1000/.645,
  cms = cms2 *lys*.645/1000
...
<<compare, fig.width = 8, fig.height = 5, fig.cap="Density curves of similarity scores from cross correlation (top) and RF scores (bottom). Different colors indicate same source versus different source. Ideally all scores of different source pairs should be much lower than scores for same source pairs.">>=
nests <- rbind(
  h44nest %>%
