...
ggplot(data = l2lscores, aes(x = land1, y = land2, fill = rfscore)) +
  geom_tile() + 
  scale_fill_gradient2("Random Forest (RF) score: ", midpoint = 0.3, low = "darkgrey", high = "darkorange", limits = c(0, 1)) +
  theme_bw() +
  xlab("Bullet 2") + ylab("Bullet 1") +
  theme(legend.position = "bottom") + coord_equal() + facet_wrap(~type)
@
