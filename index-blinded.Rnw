...
<<sam-sketch, out.width='\\textwidth', fig.width=6, fig.height = 2.5, fig.cap="Sketch of all six land-to-land sequences between two bullets with six lands.">>=
...
unknowns <- unknowns %>% ungroup() %>% mutate(
...
p3data <- all_lands %>% 
  ...
  mutate(measure = factor(measure, levels = c("ccf", "cms", "rfscore"),
         ...
         study = paste(study, group1) %>% str_trim() %>% factor(levels = c("Hamby 44", "Phoenix PD", "Houston FSI G1", "Houston FSI G2", "Houston FSI G3"))) %>%
  filter(measure == "CMS")
p3bw <- bw.nrd0(p3data$value[p3data$sameland & p3data$study == "Houston FSI G1"])
p3 <- ggplot(p3data, aes(
    x = value, fill = sameland,
    colour = sameland
  )) +
  geom_density(alpha = 0.6, bw = p3bw) +
  facet_grid(measure ~ study) +
  theme_bw() +
  scale_fill_manual("Same source", values = c("darkgrey", "darkorange")) +
  scale_colour_manual("Same source", values = c("darkgrey", "darkorange")) +
